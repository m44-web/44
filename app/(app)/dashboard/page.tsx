...
function AdminDashboard() {
  ...
  return (
    <div className="space-y-5">
      ...
      {/* Understaffed sites alert */}
      {(() => {
        const understaffed = activeSites.filter((s) => {
          if (!s.requiredGuards || s.requiredGuards === 0) return false;
          const assigned = todayShifts.filter((sh) => sh.siteId === s.id).length;
          return assigned < s.requiredGuards;
        });
        if (understaffed.length === 0) return null;
        return (
          <Link href="/sites">
            <Card className="!border-danger/30 !bg-danger/5 !py-3 flex items-start gap-3">
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="text-danger shrink-0 mt-0.5"><path d="M10.29 3.86L1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z" /><line x1="12" y1="9" x2="12" y2="13" /><line x1="12" y1="17" x2="12.01" y2="17" /></svg>
              <div className="min-w-0">
                <p className="text-sm text-danger font-semibold">本日、人員不足の現場 <span className="font-bold">{understaffed.length}件</span></p>
                <p className="text-xs text-text-secondary mt-0.5 truncate">
                  {understaffed.map((s) => {
                    const assigned = todayShifts.filter((sh) => sh.siteId === s.id).length;
                    return `${s.name}(${assigned}/${s.requiredGuards})`;
                  }).join("、")}
                </p>
              </div>
            </Card>
      ...
      {/* Guards without next-week shift requests */}
      {(() => {
        const now = new Date();
        ...
        const submittedGuardIds = new Set(
          shiftRequests.filter((r) => r.date >= nextWeekStart).map((r) => r.guardId)
        );
        const missingGuards = activeGuards.filter((g) => !submittedGuardIds.has(g.id));
        if (missingGuards.length === 0) return null;
        return (
          <Link href="/shift-requests">
            <Card className="!border-warning/30 !bg-warning/5 !py-3 flex items-start gap-3">
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="text-warning shrink-0 mt-0.5"><rect x="3" y="4" width="18" height="18" rx="2" /><line x1="16" y1="2" x2="16" y2="6" /><line x1="8" y1="2" x2="8" y2="6" /><line x1="3" y1="10" x2="21" y2="10" /></svg>
              <div className="min-w-0">
                <p className="text-sm text-warning font-semibold">来週のシフト希望未提出 <span className="font-bold">{missingGuards.length}名</span></p>
                <p className="text-xs text-text-secondary mt-0.5 truncate">
                  {missingGuards.slice(0, 6).map((g) => g.name).join("、")}
                  {missingGuards.length > 6 && ` 他${missingGuards.length - 6}名`}
                </p>
              </div>
            </Card>
          </Link>
